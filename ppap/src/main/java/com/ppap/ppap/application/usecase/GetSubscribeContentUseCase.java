...
public class GetSubscribeContentUseCase {
     ...
     */
    public SubscribeWithContentDto execute(Optional<Long> subscribeId, User user, Pageable pageable) {
        // 등록중인 subscribes
        List<Subscribe> subscribeList = subscribeReadService.getSubscribeEntityJoinNotice(user);
...
//                .findFirst().orElseThrow(() -> new Exception404(BaseExceptionStatus.SUBSCRIBE_NOT_FOUND));
        ...
        List<Content> contentList = contentReadService.findByNoticeId(curSubscribeId, pageable);
        ...
        return SubscribeWithContentDto.of(subscribeList, curSubscribeId, contentList, scrapContentIdSet);
    }
}
